...
QueryAssist.prototype.getSuccessQueen=function(backCall){
    //logger.info('开始执行终端统计');
    //扫描已经出票完成的票
    mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, collection) {
        collection.aggregate({$project:{gameCode:1,termCode:1,amount:1}},{$match: {}}, {
            $group: {
            ...
            res.sort(function(a,b){
                return b._id.termCode - a._id.termCode
            });
            var count = 0;
            var countT51 = 0;
            var countT01 = 0;
            ...
            for(var i=0; i<res.length; i++){ 
                if(res[i]._id.gameCode === 'T05' && count === 0){
                    count = 1;
                }else if(res[i]._id.gameCode === 'T05'){ 
                    res.splice(i, 1);
                    i--;
                }
            }
            for(var i=0; i<res.length; i++){ 
                if(res[i]._id.gameCode === 'T01' && countT01 === 0){
                    countT01 = 1;
                }else if(res[i]._id.gameCode === 'T01'){ 
                    res.splice(i, 1);
                    i--;
                }
            }
            for(var i=0; i<res.length; i++){ 
                if(res[i]._id.gameCode === 'T02' && countT02 === 0){
                    countT02 = 1;
                }else if(res[i]._id.gameCode === 'T02'){ 
                    res.splice(i, 1);
                    i--;
                }
            }
            for(var i=0; i<res.length; i++){ 
                if(res[i]._id.gameCode === 'T03' && countT03 === 0){
                    countT03 = 1;
                }else if(res[i]._id.gameCode === 'T03'){ 
                    res.splice(i, 1);
                    i--;
                }
            }
            for(var i=0; i<res.length; i++){ 
                if(res[i]._id.gameCode === 'T04' && countT04 === 0){
                    countT04 = 1;
                }else if(res[i]._id.gameCode === 'T04'){ 
                    res.splice(i, 1);
                    i--;
                }
            }
            for(var i=0; i<res.length; i++){ 
                if(res[i]._id.gameCode === 'T51'){
                    countT51++;
                    if(countT51 > 5){
                        res.splice(i, 1);
                        i--;
