...
TerminalAssist.prototype.printHistoryTicket = function (terminalId,ticketId,callBack) {
    log.info('---测试打印票终端机ID---' + terminalId);
    log.info('---测试打印票彩票ID---' + ticketId);
    async.waterfall([function (cb) {
        for (var i = 0; i < terminalArray.length; i++) {
        ...
        //找到需要打印的票
        mongoDBUtil.db.collection('TerminalPrintSuccess',{safe: true},function(err, _collection){
            mongoDBUtil.db.collection('HadBonusTickets', {safe: true}, function (err, collection) {
                collection.findAndModify({'ticketId': ticketId}, {'id': -1}, {$set: {status: terminalCons.ticket.status.wait}}, {new: true}, function (err, ticket) {
                    if(ticket){
                        _collection.findOne({'id':ticketId},function(err,successTicket){
                            if(successTicket.metaTicket2 && successTicket.metaTicket2Name){
                                    var headNode = {cmd: '0004', sequenceId: uuid.v4().replace(/\-/g, ''), terminalId: terminal.terminalId};
                                    var dataLen = new Buffer(successTicket.metaTicket2,'base64');
                                    var bodyNode = {
                                        packageNum: 1,
                                        nameLen: successTicket.metaTicket2Name.length,
                                        nameData: successTicket.metaTicket2Name,
                                        dataLen: dataLen.length,
                                        data: successTicket.metaTicket2
                                    };
                                    terminal.terminalControl.send0004(headNode, bodyNode);
                                    cb(null);
                            }else{
                                log.info("该打印票信息不存在，票id为："+ ticketId);
                            }
                        });
                    }else{
                        log.info("打印历史兑奖票错误信息："+ err);
                    }
                });
            });
        });
...
TerminalAssist.prototype.printBonusTicket = function (terminalId,ticketId,callBack) {
    log.info('---测试打印票终端机ID---' + terminalId);
    log.info('---测试打印票彩票ID---' + ticketId);
    async.waterfall([function (cb) {
        for (var i = 0; i < terminalArray.length; i++) {
