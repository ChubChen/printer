...
NodePlatAssist.prototype.sentP02 = function (ticket, cb) {
    log.error('返回平台打印成功:'+JSON.stringify(ticket));
    var self = this;
    var body = {};
    //根据模式选择发送的body
    var rst = [];
    ...
    if (ticket.gameCode == 'T51') {
        ...
        results.forEach(function (item) {
            ...
            if(resultTemp.indexOf('(') && resultTemp.indexOf(/其它/) > 0 ){
                pType = "03";
                pl = resultTemp.replace(/\s+/g, '').replace(/\+/g, ',').replace(/元/g, '').replace(/\:/g, '').replace(/\(/g,'' ).replace(/\)/g, '').replace(/胜其他/g,'90').replace(/平其它/g, '99').replace(/负其它/g,'09');
            }
            ...
            if(arr.length > 4){
                ...
                if(fourLen.indexOf('@') > 0){
                    ...
                    if(fourLen.indexOf('(') && fourLen.indexOf(/其它/) > 0 ){
                        pType = "03";
                        pl += fourLen.replace(/\s+/g, '').replace(/\+/g, ',').replace(/元/g, '').replace(/\:/g, '').replace(/\(/g,'' ).replace(/\)/g, '').replace(/胜其他/g,'90').replace(/平其它/g, '99').replace(/负其它/g,'09');
                    }
            ...
            _ticket.rNumber += pType + '|' + now + day + '|' + pl + ';'
        });
    }
