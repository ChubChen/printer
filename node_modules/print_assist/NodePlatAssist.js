...
NodePlatAssist.prototype.sentP02 = function (ticket, cb) {
    ...
    if (ticket.gameCode == 'T51') {
        ...
        results.forEach(function (item) {
            ...
            var pl = arr[3].replace(/\s+/g, '').replace(/\+/g, ',').replace(/元/g, '').replace(/负/g, 0).replace(/平/g, 1).replace(/胜/g, 3).replace(/\:/g, '').replace(/\(/g,'' ).replace(/\)/g, '');            _ticket.rNumber += ticket.playTypeCode + '|' + now + day + '|' + pl + ';'
        });
    }
