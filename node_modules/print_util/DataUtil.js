...
=======
var fsUtilC = require("./FsUtil.js");

>>>>>>> 0d62d672519031eefb29e61553928b6889a2ecc3
...
DataUtil.prototype.parseHeadNode = function (dataBuf) {
    ...
    try {
        ...
        headNode.cmd = dataBuf.toString(msgParam.encoding, msgParam.cmdBufIndex.start, msgParam.cmdBufIndex.end);
        headNode.sequenceId = dataBuf.toString(msgParam.encoding, msgParam.sequenceIdBufIndex.start, msgParam.sequenceIdBufIndex.end);
        headNode.terminalId = dataBuf.toString(msgParam.encoding, msgParam.terminalIdBufIndex.start, msgParam.terminalIdBufIndex.end);
    } catch (err) {
        log.info('头信息解析出错！');
        log.info(err);
        headNode = null;
...
DataUtil.prototype.handleForBodyNode = function (headNode, cmdDataBuf) {
    ...
    if (typeof self[method] === 'function') {
        ...
        log.info('找不到解析此BodyNode的方法: ' + method);
        return null;
    }
};
...
DataUtil.prototype.parse0011 = function (headNode, buf) {
    var bodyNode = {};
    bodyNode.terminalid = headNode.terminalId;
    return bodyNode;
};
...
DataUtil.prototype.parse4001 = function (headNode, buf) {
    var bodyNode = {};
    bodyNode.terminalid = headNode.terminalId;
    return bodyNode;
};
...
DataUtil.prototype.parse0002 = function (headNode, buf) {
    var bodyNode = {};
    bodyNode.retCode = buf.toString(msgParam.encoding, 0, 4);
    //转化弄成中文
   ...
   //var utf8_buffer =  Iconv.decode(tempBuf,'gb2312');

	   bodyNode.retDesc = utf8_buffer.toString(msgParam.encoding);
    return bodyNode;
};

DataUtil.prototype.parse0022 = function (headNode, buf) {
    var bodyNode = {};
    bodyNode.retCode = buf.toString(msgParam.encoding, 0, 4);
    //转化弄成中文
    var tempBuf = new Buffer(buf.length - 4);
...
DataUtil.prototype.parse0031 = function (headNode, buf) {
    ...
    try {
        ...
        index += 4;
        bodyNode.namedata1 = buf.toString(encoding, index, index + bodyNode.namelen1);
        ...
        var gb2312_to_utf8_iconv2 = new Iconv('gb2312', encoding);
        var utf8_buffer= gb2312_to_utf8_iconv2.convert(chnbuf2);
 
       //var utf8_buffer =  Iconv.decode(buf,'gb2312');

        bodyNode.data1 = utf8_buffer.toString(encoding);
        index += bodyNode.datalen1;
        bodyNode.namelen2 = buf.readInt32BE(index);
        index += 4;
        bodyNode.namedata2 = buf.toString(encoding, index, index + bodyNode.namelen2);
        ...
        var ticketFile = '/data/ticketInfo/' + fileName;
<<<<<<< HEAD
       // if(fsUtil.createDirSync(ticketFile)){
        //    fs.appendFile(ticketFile+"/successTicket.txt", JSON.stringify(bodyNode) + '\r\r', 'utf-8', function (err) {
          //      if (err) {
            //        log.info(err);
             //   } else {
              //      log.info(bodyNode.namedata1 + '已写入' + ticketFile);
               // }
           // });
      //  }
=======
        if(fsUtilC.createDirSync(ticketFile)){
            fs.appendFile(ticketFile+"/successTicket.txt", JSON.stringify(bodyNode) + '\r\r', 'utf-8', function (err) {
                if (err) {
                    log.info(err);
                } else {
...
>>>>>>> 0d62d672519031eefb29e61553928b6889a2ecc3
        ...
        log.info('0031_BodyNode转换失败,数据格式有误');
        log.info(err);
        bodyNode = null;
    }
    return bodyNode;
...
DataUtil.prototype.parse0081 = function(headNode, buf){
    var bodyNode = {};
    var gb2312_to_utf8_iconv = new Iconv('gb2312', msgParam.encoding);
    var utf8_buffer = gb2312_to_utf8_iconv.convert(buf);
	//var utf8_buffer =  Iconv.decode(buf,'gb2312');
    bodyNode.amount = utf8_buffer.toString(msgParam.encoding);
    return bodyNode;
}

DataUtil.prototype.parse0082 = function(){
    var bodyNode = {};
    return bodyNode;
}

DataUtil.prototype.parse0039 = function (headNode, buf) {
    ...
    try {
        ...
        bodyNode.data1 = utf8_buffer2.toString(encoding);
        index += bodyNode.datalen1;
        bodyNode.namelen2 = buf.readInt32BE(index);
        index += 4;
        bodyNode.namedata2 = buf.toString(encoding, index, index + bodyNode.namelen2);
...
>>>>>>> 0d62d672519031eefb29e61553928b6889a2ecc3
        ...
        log.info('0051BodyNode转换失败,数据格式有误');
        log.info(err);
        bodyNode = null;
    }
    return bodyNode;
...
//   var utf8_buffer =  Iconv.decode(tempBuf,'gb2312');
    bodyNode.retDesc = utf8_buffer.toString(msgParam.encoding)
    bodyNode.ticketId = buf.toString(msgParam.encoding, 4 + retDescLength, 4 + retDescLength + 32);
    return bodyNode;
};
...
DataUtil.prototype.parse9999 = function (headNode, buf) {
    var bodyNode = {};
    return bodyNode;
};
