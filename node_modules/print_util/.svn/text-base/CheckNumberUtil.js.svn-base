...
CheckNumberUtil.prototype.checkMut = function (ticket, data) {
    log.info("data==="+ JSON.stringify(data));
	 ...
	 log.info("ticket==="+ JSON.stringify(ticket));
    if (ticket.gameCode == 'T51') {
        ...
        var mutIndex = data.indexOf('\n倍');
        //金额的开始index
        var moneyStartIndex = data.indexOf('合计');
        //金额的结束index
        var moneyEndIndex = data.indexOf('元');
        //获取倍数
        var mutArr = data.substring(0, mutIndex).split(/\n/);
        var mut = parseInt(mutArr[mutArr.length - 1]);
        log.info('---解析出的倍数---');
        log.info(mut);
        log.info('---原始倍数---');
        log.info(ticket.multiple);
