...
TerminalControl.prototype.handle0003 = function (headNode, bodyNode) {
    var self = this;
    log.error('票据信息=============');
    log.error(JSON.stringify(bodyNode));
    if (bodyNode == null) {
                            ...
                            try {
                                ...
                                if (flag) {
                                    ...
                                    nodePlatAssist.sentP02(result, function (err, backMsgNode) {
                                        ...
                                        log.error("==========================");
                                        log.error( JSON.stringify(backMsgNode));
                                        log.error( JSON.stringify(backMsgNode.head.repCode));
                                        if (!err) {
                                            var _head = JSON.parse(backMsgNode.head);
                                            log.error(result.id + '平台状态 : ' + _head.repCode);

                                            mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, collection) {
                                                collection.update({id: result.id}, {
                                                    $set: {platStatus: _head.repCode}
                                                }, {}, function (err, res) {
                                                    log.info(result.id + '平台状态 : ' + _head.repCode);
                                                })
                                            });
                                        } else {
                                            nodePlatAssist.sentP02(result, function (err, backMsgNode) {
