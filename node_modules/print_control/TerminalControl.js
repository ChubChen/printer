                ...
                mongoDBUtil.db.collection('terminal', {
                    ...
                    collection.findAndModify({
                        ...
                        if (err) {
                            ...
                            self.io.emit('statusChange', result);
                            var _backHeadNode = {
                                cmd: '0001',
                                sequenceId: headNode.sequenceId,
                                retCode: terminalCons.retCode.success.code,
            ...
            if (terminal) {
                if (terminal.status == terminalCons.terminal.status.kill) //继续回复0001
                {
                    ...
                    if (!reset) {
                        var terminal = new ticketTask(terminal, self);
                        terminalArray.push(terminal);
                        log.error('handle0001: ' + '已重新制定指定终端机rrrrrrr' + terminal.id + '为出票');
                    }
                    ;
                        ...
                        if (err) {
                            ...
                            if (result) {
                                ...
                                if (have) {
                                    ...
                                    terminalArray.push(terminal);
									log.info("数组长度："+JSON.stringify(result));
                                    log.info('handle0001: ' + '已指定终端机' + result.value.id + '为出票');
...
TerminalControl.prototype.handle0002 = function (headNode, bodyNode) {
    var handle0002Time = moment().format('YYYY-MM-DD HH:mm:ss');
    log.info("出票请求后的响应时间是："+ handle0002Time);
    log.info('----handle0002 new----');
    log.info(headNode);
   // bodyNode = dataUtil.parse0002(headNode,bodyNode);
    log.info(bodyNode);
};

//发送福彩出票请求后的响应消息
TerminalControl.prototype.handle7500 = function (headNode, bodyNode) {
    log.info('----handle7500----');
    log.info(headNode);
   // bodyNode = dataUtil.parse0002(headNode,bodyNode);
    log.info(bodyNode);
...
TerminalControl.prototype.handle0003 = function (headNode, bodyNode) {
    var self = this;
    log.info("------handle0003 headNode------" + JSON.stringify(headNode));
    if (bodyNode == null) {
        ...
        async.waterfall([
            ...
            function (terminal, cb) {
                try {
                    ...
                    log.error('0003传来数据有误,查询来获得票' + err);
                    return;
                }
...
// 8.24 添加 ，赋值
                           result=result.value;
							...
							log.info('resultnew：' +  JSON.stringify(result));
                            var arr = bodyNode.data1.split(/\n/);
                            if (result.gameCode == 'T51') {
                                //竞彩票面信息与其他彩种不同,需要专门处理
                                result.chuan = arr[3].replace(/\D/g, '');
                                /*if(!result.chuan){
                                    result.chuan = arr[4].replace(/\D/g, '');
                                }*/
                                var tar = arr[5].replace(/\s+/g, "|");
                                arr = tar.split('|');
                                result.ticketSeq = arr[0].replace(/-/g, "");
                                result.ticketPwd2 = arr[1];
                                result.ticketPwd = arr[2];
                                /*if(!result.ticketSeq){
                                    tar = arr[6].replace(/\s+/g, "|");
                                    arr = tar.split('|');
                                    result.ticketSeq = arr[0].replace(/-/g, "");
                                    result.ticketPwd2 = arr[1];
                                    result.ticketPwd = arr[2];
                                ...
                                result.ticketSeq = arr[1].replace(/-/g, "");
                                result.ticketPwd = arr[3];
                                result.ticketPwd2 = arr[5];
                            }
                                    ...
                                    mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, collection) {
                                        collection.insert(result, function (err, data) {
                                            mongoDBUtil.db.collection('terminal', {safe: true}, function (err, collection) {
												log.info('票已被取走2'+headNode.terminalId);
                                                //修改金额
                                ////////==========金额修改了，可是状态没有修改，还是inprint(出票中)
                                                collection.findAndModify({'id': headNode.terminalId}, {'id': -1}, {
                                                    $inc: {
                                                        amount: -parseInt(result.amount) / 100,
                                                        everyCurrAmount: parseInt(result.amount) / 100
                                                    }
                                                }, {
                                                    new: true
                                                }, function (err, result) {
                                    ...
                                    nodePlatAssist.sentP02(result, function (err, backMsgNode) {
                                        //todo  处理平台返回
                                        if (!err) {
                                           // var _repCode = backMsgNode.head.repCode;
                                            var _repCode = backMsgNode.code;
                                            mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, collection) {
                                                collection.update({id: result.id}, {
                                                    $set: {platStatus: _repCode}
                                                }, {}, function (err, res) {
                                                    log.info(result.id + '平台状态 : ' + _repCode);
                                                })
                                            });
                                        } else {
                                            nodePlatAssist.sentP02(result, function (err, backMsgNode) {
                                                if (!err) {
                                                    ...
                                                    mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, collection) {
                                                        collection.update({id: result.id}, {
                                                            ...
                                                            log.error(result.id + '平台状态 : ' + _repCode);
                                                        })
                                                    });
                                                }
                                            })
                                    ...
                                    if (headNode.cmd == '0051') {
										...
										log.info("backHeadNode------------"+backHeadNode)
...
//
                                        ...
                                        //todo  处理平台返回
                                        if (!err) {
                                            mongoDBUtil.db.collection('failTicket', {safe: true}, function (err, colllection) {
                                                if (!err) {
                                                    if(!result.value){
                                                       var tResult = {};
                                                       var value = {};
                                                       tResult.value = value ;
                                                       tResult.value._id = result._id;
                                                        tResult.value.amount = result.amount;
                                                        tResult.value.auditTime = result.auditTime;
                                                        tResult.value.gameCode = result.gameCode;
                                                        tResult.value.multiple = result.multiple;
                                                        tResult.value.outerId = result.outerId;
                                                        tResult.value.termCode = result.termCode;
                                                        tResult.value.playTypeCode = result.playTypeCode;
                                                        tResult.value.betTypeCode = result.betTypeCode;
                                                        tResult.value.numbers = result.numbers;
                                                        tResult.value.id = result.id;
                                                        tResult.value.seqId = result.seqId;
                                                        tResult.value.takeTime = result.takeTime;
                                                        tResult.value.terminalId = result.terminalId;
                                                        tResult.value.terminalTakeTime = result.terminalTakeTime;
                                                        if(result.metaTicket){
                                                            tResult.value.metaTicket = result.metaTicket;
                                                        }
                                                        collection.insert(tResult, function () {
                                                           log.error(tResult.value.id + '已入失败库');
                                                           self.io.emit('update-fail-ticekt-list',tResult);
                                                           self.io.emit('update-fail-ticekt-list-global',tResult);
                                                        });
                                                    }else{
                                                       colllection.insert(result, function () {
                                                            log.error(result.value.id + '已入失败库');
                                                            self.io.emit('update-fail-ticekt-list',result);
                                                            self.io.emit('update-fail-ticekt-list-global',result);
                                                       });
                                                    }
                                                }
                                            });
...
TerminalControl.prototype.handle0039 = function (headNode, bodyNode) {
    ...
    if (bodyNode == null) {
        ...
        async.waterfall([
            ...
            function (terminal, cb) {
                try {
                    if (terminal == null || terminal.terminalId == null || terminal.terminalName == undefined) {
                ...
                mongoDBUtil.db.collection('waitTerminal' + terminal.terminalName, {
                    ...
                    collection.findAndRemove({
                        ...
                        if (!result) {
                            ...
                            ;
                            try {
								//checkNumberUtil.checkMut(result, bodyNode.data1);
                                var flag = true;
                                if (flag) {
                                    mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, collection) {
            ...
            mongoDBUtil.db.collection('terminal', {
                ...
                collection.findAndModify({
                    ...
                    if (!reset) {
                        var terminal = new ticketTask(result, self);
                        terminalArray.push(terminal);
                        log.error('handle0001: ' + '已重新制定指定终端机' + terminal.id + '为兑奖');
                    };
                    cb(null);
                })
            })
...
TerminalControl.prototype.handle0071 = function (headNode, bodyNode) {
    ...
    mongoDBUtil.db.collection('TicketsWaitBonus', {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            log.info('_bonusInfo :第' + (i + 1) + '张   ' + _bonusInfo);

            /*if (_bonusInfo.indexOf('奖期不在可兑奖状态') >= 0 || _bonusInfo.indexOf('未开奖') >= 0 || _bonusInfo.indexOf('未开始兑奖') >= 0 || _bonusInfo.indexOf('其它省中心的票')) { //不在兑奖时间内
                var seq = _bonusInfo.split('#')[0];
            ...
            log.info("是否包含_bonusInfo.indexOf('中奖金额')"+_bonusInfo.indexOf('中奖金额'));
            log.info("是否包含_bonusInfo.indexOf('恭喜中奖')"+_bonusInfo.indexOf('恭喜中奖'));
            if (_bonusInfo.indexOf('可以兑奖') >= 0) {
                var seq = _bonusInfo.split('#')[0];
                log.info(seq +'恭喜中奖');
                self.insert2HistoryOld(headNode.terminalId, _bonusInfo, seq);
            }else if (_bonusInfo.indexOf('中奖金额') >= 0) {
                var seq = _bonusInfo.split('#')[0];
                log.info(seq +'竞彩+恭喜中奖');
                self.insert2HistoryOld(headNode.terminalId, _bonusInfo, seq);
            } else if (_bonusInfo.indexOf('未中奖') >= 0) {
                var seq = _bonusInfo.split('#')[0];
                log.info(seq +'未中奖');
                //可专门放一个未中奖库
                collection.findAndModify({
                ...
                collection.findAndModify({
                    ...
                    $set: {
                        status: terminalCons.ticket.status.bonus,
                        bonusInfo: _bonusInfo
                    }
                }, {
                    new: true
                }, function (err, result) {
                    log.info(seq + "返回未知兑奖状态,页面查看原因:"+_bonusInfo);
                })
            }
        }
    })
...
TerminalControl.prototype.handle0032 = function (headNode, bodyNode) {
    ...
    //首先获取该票所在的库
    log.info(headNode);
    //bodyNode = dataUtil.parse0032(headNode,bodyNode);
    log.info(bodyNode);
        ...
        collection.findOne({'id': headNode.terminalId}, function (err, terminal) {
            if (terminal) {
                ...
                mongoDBUtil.db.collection(name, {safe: true}, function (err, _coll) {
                    _coll.findOneAndDelete({'id': ticketId}, [], function (err, _ticket) {
                        mongoDBUtil.db.collection('failTicket', {safe: true}, function (err, __coll) {
                            if (!_ticket) {
                                ...
                                __coll.insertOne(_ticket, function () {
                                    self.io.emit('update-fail-ticekt-list-global',_ticket);
                                });
...
TerminalControl.prototype.handle4001 = function (headNode, bodyNode) {
    ...
    for (var i = 0; i < terminalArray.length; i++) {
        ...
        if (terminal.terminal.id == terminalId) {
            ...
            //log.error(msg);
            self.io.emit('statusChange', terminal.terminal);
        }
    }
};
    ...
    mongoDBUtil.db.collection('terminal', {safe: true}, function (err, collection) {
        collection.findAndModify({id: headNode.terminalId}, {}, {$set: {status: terminalCons.terminal.status.notAvailable}}, {new: true}, function (err, terminal) {
            ...
            if (terminal) {
                ...
                self.io.emit('statusChange', terminal);
    ...
    mongoDBUtil.db.collection('TicketsWaitBonus', {
        ...
        collection.findAndRemove({
            ...
            if (result) {
                ...
                result = result.value;
                log.info("中奖票result=" + JSON.stringify(result));
                if(result){
                //delete result['_id'];
                ...
                if(result.gameCode == 'T05'){ //11选5
                    var preStr = bonusInfo.split(/\r\n/)[2].split('奖金')[1];
                    var aftStr = bonusInfo.split(/\r\n/)[3];
                    preBouns = preStr.replace(/[^\d\.]/g, '');
                    aftBouns = aftStr.replace(/[^\d\.]/g, '');
                }if(result.gameCode == 'T01' || result.gameCode == 'T02' || result.gameCode == 'T03' || result.gameCode == 'T04'){ //数字彩
                    var bounsArr = bonusInfo.split(/\n/)[1].split('元');
                    preBouns = bounsArr[0].replace(/[^\d\.]/g, '');
                    aftBouns = bounsArr[2].replace(/[^\d\.]/g, '');
                }else if(result.gameCode == 'T51'){//竞彩足球
                    var bounsArr = bonusInfo.split(/\n/);
                    if(bonusInfo.indexOf('旧')>=1){
                       aftBouns = bounsArr[7].replace(/[^\d\.]/g, '');
                       //result.terminalId = bounsArr[5].replace(/[^\d\.]/g, '');//兑奖终端
                       //var s = bounsArr[6].replace(/[^\d\-\:\s]/g, '');
                       //result.bonusTime = s.substring(1,s.length);//兑奖时间
                    }else{
                       aftBouns = bounsArr[5].replace(/[^\d\.]/g, '');
                    }
                    preBouns = bounsArr[2].replace(/[^\d\.]/g, '');
                }
            
                if (bonusInfo.indexOf('上级兑奖机构兑奖') != -1) {
                    result.bonusInfo = bonusInfo;
                    //result.createTime = new Date().getTime();
                    //result.bonusTime = moment().format('YYYY-MM-DD HH:mm:ss');
                    result.bonusTime = new Date().getTime();
                    result.preBouns = parseFloat(preBouns);
                    result.aftBouns = parseFloat(aftBouns);
                    result.platBounsStatus = '0000';
                    result.status = terminalCons.ticket.status.bigBonus;
                    result.cashTerminal = terminalId;

                    mongoDBUtil.db.collection('HadBonusTickets', {
                        safe: true
                    }, function (err, _coll) {
                        _coll.insert(result, function () {
                            log.info(result.ticketSeq + '--大奖--已入Had兑奖库');
                        })
                    })
                } else {
                    //result.createTime = new Date().getTime();
...
TerminalControl.prototype.insert2HistoryOld = function (terminalId, bonusInfo, seq) {
    mongoDBUtil.db.collection('TicketsWaitBonus', {
        safe: true
    }, function (err, collection) {
        collection.findAndRemove({
                ...
                if (bonusInfo.indexOf('上级兑奖机构兑奖') != -1) {
                    ...
                    async.each(arr, function (item) {
                        if (item.indexOf('合计') != -1) {
                            var  tempBonus = parseInt(item.replace(/\D/g, ''));
                            if(!isNaN(tempBonus)){
                                realBonus = tempBonus;
                            }
                        }
...
TerminalControl.prototype.failHandler = function (headNode, bodyNode) {
    ...
    mongoDBUtil.db.collection('terminal', {safe: true}, function (err, collection) {
        collection.findOne({'id': headNode.terminalId}, function (err, terminal) {
            if (terminal) {
                mongoDBUtil.db.collection('waitTerminal' + terminal.terminalName, {safe: true}, function (err, _coll) {
                    _coll.findOneAndDelete({'id': ticketId}, {}, function (err, ticket) {
                        if (ticket) {
                            mongoDBUtil.db.collection('failTicket', {safe: true}, function (err, __coll) {
                                delete ticket['_id'];
                                //新增失败信息,失败代码
                                ...
                                //入失败库
                                __coll.insertOne(ticket,{}, function () {
                                    self.io.emit('update-fail-ticekt-list-global',ticket);
                                });
                                //通知终端机失败
                                log.info('---发送0055---');
                                var backHeadNode0005 = {
...
TerminalControl.prototype.send0002 = function (headNode, bodyNode) {
    var self = this;
    log.info("----------send0002 new-----------");
    var send0002StartTime = new Date().getTime();
    var startSend0002 = moment().format('YYYY-MM-DD HH:mm:ss');
    log.info("出票请求的开始时间startSend0002是："+ startSend0002);
    async.waterfall([
        function (cb) {
            /**
             * 第一步,计算总长度
             * 1.headNode长度为headLen+packageNumLen4
             * 2.bodyNode的长度为12+namedata+data的
             */
            var totalLength = msgParam.headBufLen + msgParam.lengthBufLen;
            for (var i = 0; i < bodyNode.tickets.length; i++) {
                totalLength += msgParam.lengthBufLen + bodyNode.tickets[i].namelen + msgParam.lengthBufLen + bodyNode.tickets[i].datalen;
            }
            cb(null, totalLength)
        },
        function (totalLength, cb) {
            ...
            buf.writeInt32BE(totalLength - msgParam.lengthBufLen, 0);
            result.copy(buf, msgParam.lengthBufLen, 0, result.length);
            //写入总的包个数
            result = new Buffer(msgParam.lengthBufLen);
            result.writeUInt32BE(bodyNode.tickets.length, 0);
            result.copy(buf, msgParam.headBufLen, 0, result.length);
        ...
        function (buf, cb) {
            self.sendBuf(buf);
            var send0002EndTime = new Date().getTime();
            var endSend0002 = moment().format('YYYY-MM-DD HH:mm:ss');
            log.info("出票请求的结束时间endSend0002是："+ endSend0002);
            log.info("send0002EndTime-send0002StartTime = " + (send0002EndTime-send0002StartTime) + "ms");
 ...
 * 
 */
TerminalControl.prototype.send7500 = function (headNode, bodyNode) {
    var self = this;
    async.waterfall([
        function (cb) {
            /**
        ...
        function (totalLength, cb) {
            //先写头
            var head = 7500 + headNode.sequenceId + headNode.terminalId;
            var result = new Buffer(head, msgParam.encoding);
            var buf = new Buffer(totalLength);
            buf.writeInt32BE(totalLength - msgParam.lengthBufLen, 0);
            result.copy(buf, msgParam.lengthBufLen, 0, result.length);
...
TerminalControl.prototype.send0004 = function (headNode, bodyNode) {
    ...
    async.waterfall([
        function (cb) {
            /**
             * 第一步,计算总长度
             * 1.headNode长度为headLen+lengthBufLen
             * 2.bodyNode长度为numberPackageLen+dataLen + nameLen + 4 + 4
             */
            var totalLength = msgParam.headBufLen + msgParam.lengthBufLen + bodyNode.nameLen + bodyNode.dataLen + msgParam.lengthBufLen + msgParam.lengthBufLen;
            cb(null, totalLength)
        },
        function (totalLength, cb) {
            //先写头
            ...
            result.copy(buf, index, 0, bodyNode.nameLen);
            index += bodyNode.nameLen;
            //写入数据长度
            result = new Buffer(4);
            ...
            result.copy(buf, index, 0, bodyNode.dataLen);
            cb(null, buf);
        },
        function (buf, cb) {
            self.sendBuf(buf);
...
TerminalControl.prototype.send0005 = function (headNode, bodyNode) {
     ...
     * body : ID长度：32
     */
    var totalLength = msgParam.headBufLen + 32;
    var head = headNode.cmd + headNode.sequenceId + headNode.terminalId;
    var result = new Buffer(head, msgParam.encoding);
    ...
    buf.writeInt32BE(totalLength - msgParam.lengthBufLen, 0);
    result.copy(buf, msgParam.lengthBufLen, 0, result.length);
    if(bodyNode.ticketid && (bodyNode.ticketid + '').length < 32){
        var length = 32 - (bodyNode.ticketid + '').length;
        for (var i = 0; i < length; i++) {
            bodyNode.ticketid='0' +bodyNode.ticketid;
...
TerminalControl.prototype.handle0081 = function (headNode, bodyNode) {
    ...
    try{
        ...
        for (var i = 0; i < terminalArray.length; i++) {
            ...
            if (terminal.terminal.id == terminalId) {
                terminal.amount = bodyNode.amount;
                mongoDBUtil.db.collection('terminal', {
                    safe: true
                }, function (err, collection) {
                    collection.findAndModify({
                        ...
                        if (err) {
                            ...
                            self.io.emit('changeAmount', JSON.stringify(result));
                            log.info(result);
                        }
                    })
                });
...
TerminalControl.prototype.send0009 = function (headNode, bodyNode) {
     ...
     * body : 4
     */
    var totalLength = msgParam.headBufLen + 4;
    var head = headNode.cmd + headNode.sequenceId + headNode.terminalId;
    var result = new Buffer(head, msgParam.encoding);
...
TerminalControl.prototype.send0007 = function (headNode, bodyNode) {
    ...
    async.waterfall([
        function (cb) {
            /**
             * 第一步,计算总长度
             * 1.headNode长度为headLen不变,为包长+headNodeLen
             * 2.bodyNode长度为包数量4 + nameLen4 + nameData  +dataLen4 + data   永远一个包，data中多个兑奖数据
             */
            var totalLength = msgParam.headBufLen + msgParam.lengthBufLen + msgParam.lengthBufLen + bodyNode.metaTicketName.length + msgParam.lengthBufLen;
            var dataLength = 0;
            for (var i = 0; i < bodyNode.tickets.length; i++) {
        ...
        function (totalLength, dataLength, cb) {
            var buf = new Buffer(totalLength);
            var index = msgParam.lengthBufLen;
            //写入包长
            buf.writeInt32BE(totalLength - msgParam.lengthBufLen, 0);
            //写入头
            var head = headNode.cmd + headNode.sequenceId + headNode.terminalId;
            ...
            result.copy(buf, index, 0, result.length);
            index += result.length;
            cb(null, buf, dataLength, index);

        },
        function (buf, dataLength, index, cb) {
            var result;
            for (var i = 0; i < bodyNode.tickets.length; i++) {
                //写入数据内容
...
TerminalControl.prototype.send8000 = function (headNode, bodyNode) {
    ...
    async.waterfall([
        function (cb) {
             ...
             * 第一步,计算总长度
             * 1.headNode长度为56不变
             * 2.bodyNode长度为 60 + 4 + 4 + name
             */
            var totalLength = msgParam.headBufLen + msgParam.lengthBufLen + msgParam.lengthBufLen + msgParam.lengthBufLen + name.length;
            var data = "";
...
TerminalControl.prototype.handle8001 = function () {
    log.info("不用处理:handle8001");
}

TerminalControl.prototype.handle8002 = function () {
    log.info("不用处理:handle8002");
}
...
mongoDBUtil.init(function(){
        terminalControl.insert2HistoryOld("","","20197984205926202371")
});*/
