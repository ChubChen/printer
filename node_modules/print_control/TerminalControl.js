...
TerminalControl.prototype.handle9999 = function(){
    ...
    async.waterfall([
        ...
        function(cb){
            for(var i =0 ; i< terminalArray.length; i++){
                terminalArray[i].status = terminalCons.terminal.kill;
                terminalArray[i].terminal.status = terminalCons.terminal.kill;
            }
            cb(null);
        },
        function(cb){
            ...
            async.each(terminalArray, function(terminal, callback){
                mongoDBUtil.db.collection('terminal', {safe : true}, function(err, conn){
                    conn.findAndModify({id: terminal.terminalId},{$set:{status: terminalCons.terminal.kill}},{},function(err, data){
        ...
        log.info("向页面发送ErrorStaus");
        webIo.io.emit('ErrorStatus', terminalCons.terminal.kill);
        if(err)
        log.error(err)
    })
}
