...
SocketControl.prototype.querySuccessTickets = function (bodyNode, cb) {
    mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, collection) {
        ...
        collection.find(bodyNode.cond, {"gameCode": 1, "termCode": 1, "id": 1, "numbers": 1, "status": 1}).sort({"id": -1}).skip(skip).limit(bodyNode.limit).toArray(function (err, datas) {
...
SocketControl.prototype.queryHistoryTickets = function (bodyNode, cb) {
    mongoDBUtil.db.collection('History', {safe: true}, function (err, collection) {
        var skip = (bodyNode.curPage - 1) * bodyNode.limit;
        collection.find(bodyNode.cond, {"gameCode": 1, "termCode": 1, "id": 1, "numbers": 1, "status": 1,"bonus":1}).sort({"id": -1}).skip(skip).limit(bodyNode.limit).toArray(function (err, datas) {
            collection.count(bodyNode.cond, {}, function (err, count) {
                var backNode = {};
                backNode.count = count;
                backNode.curPage = bodyNode.curPage;
