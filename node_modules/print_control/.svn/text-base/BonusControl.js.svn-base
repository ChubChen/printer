...
//读取兑奖文件定时任务
//*/300 * * * * *
BonusControl.prototype.getFTPBounsTickets = new CronJob('30 51 16 * * *', function () {
    log.error('*******兑奖****** ftp任务');
    ...
    //}
    bonusControl.getBonusFileNew('/data/app/issue');
    ...
    async.waterfall([function (cb) {
        ...
        mongoDBUtil.db.collection('TicketsWaitBonus', {safe: true}, function (err, collection) {
            ...
            for (var i = 0; i < gameCodeArr.length; i++) {
                ...
                //todo 测试写死兑奖期次
                if (terminal.gameCode[gc] && terminal.area) {
                    collection.find({
...
BonusControl.prototype.dealWithBonuTicket = new CronJob('0 55 16 * * *', function () {
            ...
            async.eachSeries(infos, function (infoLine, termCallBack) {
                ...
                try {
                    if (infoLine.trim() != '') {
                        ...
                        var ticketSeq = strArr[0];
                        /*var length = 32 - (id + '').length;
                        for (var i = 0; i < length; i++) {
                            id = '0' + id;
                        }*/
                        var ticketPwd = strArr[1];
                        var gameCode = strArr[2];
                        var termCode = strArr[3];
                       
                        var  metaTicketName = strArr[4];
                        var ticketId = strArr[5];
                        var outerId = strArr[6];
                        var multiple = strArr[7];
                        var amount = strArr[8];
                        var numbers = strArr[9];
                        var ticket = {};
                        ticket.ticketSeq = ticketSeq;
                        ticket.ticketPwd = ticketPwd;
                        ticket.gameCode = gameCode;
                        ticket.metaTicketName = metaTicketName;
                        ticket.termCode = termCode; //期次
                        ticket.bonusTime = now;
                        ticket.ticketId = ticketId;
                        ticket.outerId = outerId;
                        ticket.multiple = multiple;
...
//回传票奖金到出票平台
BonusControl.prototype.sendBouns2NodePlat = new CronJob('0 */2 * * * *', function () {
    ...
    log.error("*******回推中奖金额******");
        ...
        mongoDBUtil.db.collection('HadBonusTickets', {safe: true}, function (err, collection) {
               collection.find({platBounsStatus:{$in:['0000',null]}}).toArray(function(err,docs){
                    if(docs.length >=1){
                      async.each(docs, function(item){
                          if(item.outerId != null && item.preBouns != null && item.aftBouns !=null){
