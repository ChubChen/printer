...
//读取兑奖文件定时任务
//*/300 * * * * *
BonusControl.prototype.getFTPBounsTickets = new CronJob('*/30 * * * *', function () {
    log.error('*******兑奖****** ftp任务');
...
BonusControl.prototype.dealWithBonuTicket = new CronJob('*/10 * * * *', function () {
...
//回传票奖金到出票平台
BonusControl.prototype.sendBouns2NodePlat = new CronJob('*/30 * * * * *', function () {
    log.error("*******回推中奖金额******");
        //1100:未回推
        //2100:已回推
                    ...
                    if(docs.length >=1){
                        async.each(docs, function(item) {
                            if (item.id != null && item.aftBouns != null) {
                                ...
                                tempItem.cashAmount = item.aftBouns;
                                tempItem.cashTime = item.bonusTime;
                                tempItem.id = item.id;
                                var sendBody = {rst:new Array()};
                                sendBody.rst.push(tempItem);
                                nodePlatAssist.sendBounsTickets(sendBody, function (err, backMsgNode) {
                                    if (err) {
                                        log.info(err);
                                    } else {
                                        log.info(backMsgNode.repCode);
                                        if (backMsgNode.repCode == '0000') {
                                            collection.update({id: item.id}, {$set: {platBounsStatus: '1111'}}, {new: true}, function (err, result) {
                                                if (!err) {
