...
//读取兑奖文件定时任务
//*/300 * * * * *
BonusControl.prototype.getFTPBounsTickets = new CronJob('0 0 */1 * * ', function () {
    log.error('*******兑奖****** ftp任务');
...
BonusControl.prototype.dealWithBonuTicket = new CronJob('0 */5 * * *', function () {
...
//回传票奖金到出票平台
BonusControl.prototype.sendBouns2NodePlat = new CronJob('*/2 * * * *', function () {
    ...
    log.error("*******回推中奖金额******");
        ...
        mongoDBUtil.db.collection('HadBonusTickets', {safe: true}, function (err, collection) {
               collection.find({platBounsStatus:{$in:['0000',null]}}).toArray(function(err,docs){
                    if(docs.length >=1){
                        async.each(docs, function(item) {
                            if (item.ticketId != null && item.aftBouns != null) {
                                var tempItem = {};
