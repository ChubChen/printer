...
mongoDBUtil.init(function(){
    ...
    BonusControl.prototype.getFTPBounsTickets.start();

})


BonusControl.prototype.getFTPBounsTickets = new CronJob('*/5 * * * * *', function () {
    log.error('*******兑奖****** ftp任务');
                                    ...
                                    async.eachSeries(ftps, function (index, _callback) {
                                        ...
                                        fs.readFile(terminalCons.ftpTicketFileDir + name, 'UTF-8', function (err, data) {
                                            ...
                                            if (!err) {
                                                log.error(data);
                                                var arr = data.split('\n');
                                                async.eachSeries(arr, function (item, __callback) {
                                                    try {
                                                        if (item.trim() != '') {
...
BonusControl.prototype.bonusTickets = new CronJob('*/5 * * * * *', function () {
    log.error('*******兑奖****** 兑奖任务');
