...
//读取兑奖文件定时任务
//*/300 * * * * *
BonusControl.prototype.getFTPBounsTickets = new CronJob('*/10 * * * * *', function () {
    log.error('*******兑奖****** ftp任务');
    ...
    log.error("*******回推中奖金额******");
        ...
        mongoDBUtil.db.collection('HadBonusTickets', {safe: true}, function (err, collection) {
               collection.find({platBounsStatus:{$in:['0000',null]}}).toArray(function(err,docs){
                    if(docs.length >=1){
                        async.each(docs, function(item) {
                            if (item.ticketId != null && item.preBouns != null && item.aftBouns != null) {
                                var tempItem = {};
...
mongoDBUtil.init(function(){
    bonusControl.dealWithBonuTickets.start;
});
