    ...
    new CronJob('*/5 * * * * *', function () {
        ...
        async.waterfall([function (cb) {
            ...
            for (var i = 0; i < terminalArray.length; i++) {
                log.error(terminalArray[i].terminalId + terminalArray[i].status);
                                        ...
                                        fs.readFile(terminalCons.ftpTicketFileDir + name, 'UTF-8', function (err, data) {
                                            if (!err) {
                                                ...
                                                async.each(arr, function (item, callback) {
                                                    ...
                                                    collection.findAndRemove({gameCode: term.gameCode, termCode: term.termCode}).toArray(function (err, _tickets) {
                                                        async.each(_tickets, function (ticket, hisBackCall) {
