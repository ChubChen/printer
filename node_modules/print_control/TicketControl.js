...
TicketControl.prototype.sendPrintTicket = new CronJob('*/4 * * * * *', function () {
    log.info('#########出票######### 出票任务');
    async.waterfall([function (cb) {
        ...
        if (terminal.status == terminalCons.terminal.status.available) {
            mongoDBUtil.db.collection('terminal', {safe: true}, function(err, collection){
                collection.findOne({"id": terminal.id}, function (err, dataTerminal) {
                    if(err){
                        cb(err);

                    }else{
                        log.info("查找到需要出票的终端机：" + dataTerminal);
                        if(dataTerminal.preAmount == undefined || dataTerminal.preAmount == null){
                            dataTerminal.preAmount = 0 ;
                        }
