...
//新的发送票给终端机逻辑
TicketControl.prototype.newSendPrintTicket = new CronJob('*/1 * * * * *', function () {
    var self = this;
    log.info('#########体彩出票######### 出票任务');
    ...
    async.waterfall([
        function(cb){
            ...
            if(dataTerminal.gameCode){
                ...
                for(dataTerminal.gameCode in key){
                    gameCode.push(key);
                }
            ...
            if(_ticket){
                ...
                mongoDBUtil.db.collection('waitTerminal' + terminal.terminalName, {safe: true}, function (err, collection) {
                    collection.insertOne(_ticket,{}, function () {
                        log.info(ticket.id + '已进入' + terminal.terminalName + '等待处理库');
                        var headNode = {};
                        headNode.cmd = '0002';
...
//将出票成功且没有及时返回的票再次给平台
TicketControl.prototype.dealPrintSuccessToPlat = new CronJob('*/1 * * * *', function () {
                ...
                async.eachSeries(tickets, function (result, cb) {
                    ...
                    nodePlatAssist.sentP02(result, function (err, backMsgNode) {
                        ...
                        if (!err) {
                            ...
                            mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, _coll) {
                                _coll.update({id: result.id}, {
                            ...
                            mongoDBUtil.db.collection('TerminalPrintSuccess', {safe: true}, function (err, scuccessC) {
                                scuccessC.update({id: result.id}, {
                                    $set: {platStatus: "0000"}
                                }, {}, function (err, res) {
